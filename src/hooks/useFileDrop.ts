      ...
      try {
        ...
        unlistenFileDrop = await listen('tauri://file-drop', async (event) => {
          ...
          try {
            ...
            if (!isValid) {
              setError(new Error('Please drop an image file (JPG, PNG, WebP, GIF)'));
            ...
            // Set current image
            const imageIndex = folderImages.findIndex(img => img.path === filePath);
            if (imageIndex !== -1) {
              setCurrentImage(filePath, imageIndex);
              console.log('Set current image:', filePath, 'at index:', imageIndex);
            }
        ...
        unlistenFileDropCancelled = await listen('tauri://file-drop-cancelled', (event) => {
