 ...
 *
 * Module-level cache of decoded bitmaps (hypothesis C — rationale in
 * docs/superpowers/specs/2026-08-16-nav-rapid-bitmap-window-design.md),
 * keyed by path and tier. Keeps decoded pixels alive independent of the
 * renderer's own image-cache eviction, so a preload-hit navigation can
 * paint without re-decoding. Not part of the Zustand store: ImageBitmap
 * objects are large mutable resources, not immutable state. Eviction
 * POLICY lives in useImagePreloader (it knows index/direction/budget);
 * this module only does bookkeeping and deterministic release via
 * close().
 *
 ...
 * upgrade decodes in the background. Callers that only care about "the
 * best bitmap available" (the pre-§9-Phase-3 API) use `getBitmap`/
 * `getRetained`, which prefer full over preview.
 */
