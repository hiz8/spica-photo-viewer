...
describe('FileOpenButton', () => {
  ...
  it('should open image when file is selected', async () => {
    const mockFolderImages = [
      { path: '/test/selected-image.jpg', name: 'selected-image.jpg', size: 1024, modified: Date.now() }
    ];
    ...
    await vi.waitFor(() => {
      expect(mockInvoke).toHaveBeenCalledWith('get_folder_images', { path: '/test' });
      expect(mockStore.setFolderImages).toHaveBeenCalledWith('/test', mockFolderImages);
      expect(mockStore.setCurrentImage).toHaveBeenCalledWith('/test/selected-image.jpg', 0);
    });
  });
  ...
  it('should handle rapid clicks', async () => {
    const mockFolderImages = [
      { path: '/test/image.jpg', name: 'image.jpg', size: 1024, modified: Date.now() }
    ];
