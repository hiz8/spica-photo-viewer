...
7. **（提案・未実施）Spica 固有の引き金を絞る**（§1.5 の別アプリ対照を受けて）。窓が現れて +30ms 前後に起きるので、それまでに Spica がしていることが候補になる。判別力の高い条件（その日まだ開いていない NAS フォルダへ移動して最初の起動、Spica で ~30%）で、次の順に 1 仮説ずつ確かめる。
