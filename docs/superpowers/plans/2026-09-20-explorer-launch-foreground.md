...
- **Spica 固有（2026-09-21 夕方、Picasa を同じ条件で起動した対照で確定。0/40 対 8/40）。何が引き金かは未確定**（下記「行為者の判別実験」）。いったん「プローブが必要条件」と判断したが、2026-09-21 夕方に訂正した。実験の条件が発生率の低いものだったので判別力が無く、その後 C3 のビルドでプローブより前に持ち上げが起きたので、プローブは引き金ではない。当初の候補は、エクスプローラー/OS 自身か、+6〜100ms で重なる自前の `explorer_sort` COM 読み取り（IShellWindows、読み取りのみ）が誘発したか、の 2 つだった。対照実験（2026-09-21、ユーザー実施）: `.txt` をメモ帳で 30 回ダブルクリック起動してもエクスプローラーはメモ帳の上に来なかった。日常使用でも Spica 以外でこの事象は見ていない。したがってエクスプローラー/OS の一般挙動である可能性は低く、Spica が起動直後に行う処理のうちエクスプローラーの UI スレッドに届くのは COM プローブだけなので、それが第一容疑。対策 C4 は行為者に依存しない。
...
- **C3 の実機 A/B（2 回目、1 ブロック目で打ち切り）**: C3 の 10 起動（同じ UNC フォルダで続けて起動）のうち 1 回（10 回目）で持ち上げが起きた。ウォッチャーでは窓が現れて +23ms、覆われていた時間は 437ms（W3 が `window_created` で直した）。`window_created` の `z_above` も起動元だった。C3 では走査とプローブが `window_created` より後に始まるので、**プローブが走る前に持ち上げが起きている**。したがって**起動時プローブは引き金ではない**（必要条件でもない）。C3 は事象を防げず、代償だけが残るので A/B を打ち切って revert した（`docs/code-rationale.md` W4 に非採用として記録）。
- **メモ帳対照の読み直し**: 「Spica 固有」の根拠だったメモ帳 30 起動は、条件（同じフォルダの繰り返し起動か、フォルダを移動した直後か）が記録されていない。Spica でも繰り返し起動の条件では ~3% なので、メモ帳 0/30 は Spica 固有であることの証拠にならない。Spica 固有かどうかも未確定。次は、同じ条件（NAS フォルダ）で別アプリを起動してエクスプローラーが持ち上がるかを数える。
- **別アプリ対照（2026-09-21 夕方、Picasa Photo Viewer）**: `.jpg` の既定アプリを切り替え、同じエクスプローラー窓・同じ手順で起動した。持ち上げは、各アプリの窓を対象にした 15ms ウォッチャーで数えた（Spica は `perf.log` の `z_above` とも一致）。Picasa の窓（`ytWindow`）は topmost ではないので、エクスプローラーはその上に来られる。
  - 同じ UNC フォルダで続けて起動（~2 秒間隔）: Spica 2/20、Picasa 0/20（Spica が先、Picasa が後）。
  - その日まだ開いていない NAS フォルダへ移動して最初の起動（毎回別フォルダ、~5 秒間隔）: 10 起動ずつ Picasa → Spica → Picasa → Spica と交互にし、**Spica 6/20（1/10 と 5/10）、Picasa 0/20**。Fisher 片側 p ≈ 0.010。
  - 合計すると Spica 8/40、Picasa 0/40（p ≈ 0.003）。Spica の持ち上げはすべて、窓が現れて +29〜54ms。
  - **判定: この事象は Spica 固有**（同じ条件で Picasa では起きない）。限界として、Picasa が自分の窓を 15ms 以内に持ち上げ直していれば、ウォッチャーは見逃す。
  - 起動時プローブは引き金ではない（C3）ので、残る Spica 固有の違いは、窓が現れてから +30ms 前後までに Spica がしていることの中にある。窓を最初から最大化で作る（tao の `SW_MAXIMIZE` → `SW_SHOW`、#310 で導入）、WebView2 の生成（`msedgewebview2.exe` 子プロセスの起動）、サムネイル先読みのファイル I/O など。§3 の 7（提案）。
...
1. **C1: 起動直後の前面再主張**（2026-09-21 に撤去。実機 260 起動で前面が失われた起動は 0 件、発動も 0 回。報告の症状は W3 の状態だった。`docs/code-rationale.md` W2）（§1.4(a) で失われた 2 回目のアクティブ化の復元。H1/H4/H5 を同時にカバー、低リスク）。起動ファイルありのときだけ、`window_created` 直後・`page_load_finished`・`WindowEvent::Focused(false)` の各契機で `GetForegroundWindow() != hwnd` なら `SetForegroundWindow(hwnd)`。`run_start` から 1500ms を過ぎたら、または 2 回試したら何もしない。結果と `GetLastError` を perf ログへ。`maximize_window` を非最大化に戻して旧挙動を再現する案は、#310 の起動高速化（800×600 → 最大化のジャンプ回避）を捨てることになるので採らない。
...
6. **起動時プローブをウインドウ表示と重ねない**（2026-09-21 に C3 を実装したが、非採用で revert。C3 でもプローブより前に持ち上げが起きた。§1.5、W4）。提案時は、§1.5 の実験 A/B から「z の持ち上げはプローブと表示が重なったときだけ起きる」と判断していた。この判断は同日夕方に訂正した（A/B に判別力が無かった）。現在は、重なりをなくせば消えるかを C3 自体で確かめる位置づけ。候補は 2 つある。
   ...
   - （結果）C3 でもプローブより前に持ち上げが起きたので、プローブは引き金ではない。局所修正も対象が無くなった。
7. **（提案・未実施、PR #332 の後に別 PR で行う）Spica 固有の引き金を絞る**（§1.5 の別アプリ対照を受けて）。窓が現れて +30ms 前後に起きるので、それまでに Spica がしていることが候補になる。判別力の高い条件（その日まだ開いていない NAS フォルダへ移動して最初の起動、Spica で ~30%）で、次の順に 1 仮説ずつ確かめる。
   - **#310 より前のビルド**（窓は 800×600 の非最大化で生成し、~500ms 後に `maximize_window` で最大化）をインストール先に入れ替え、ウォッチャーで +30ms の持ち上げが出るかを見る。出るなら最大化生成は無関係で、#310 以前は ~0.5 秒後の `SW_MAXIMIZE` が隠していただけ（W3 と同じ）。出ないなら最大化生成が第一容疑。
   - 最大化生成が容疑なら、窓を非表示で作り、WebView2 生成の後で表示するなどの実験ビルドを作る。WebView2 の子プロセス起動が容疑なら、表示との順序を入れ替える。
   - いずれも起動時間と見た目（#310 で消した 800×600 からのジャンプ）に影響するので、`profile-startup.mjs` で測って採否を決める。W3 はどれを採っても保険として残す。

実施順は **C1 → A1 → A2 → A3 → C4**（C1 は根拠が確定しており単独で価値があるので先に出す。A1–A3 は C1 で解消しない場合の切り分け用。C4 は A1–A3 の採取結果から決まった）。C4 の後に実験 A/B（§1.5、2026-09-21）を行ったが、発生率の低い条件だったので判別力が無かった。6 の C3 を実装したが（`835fd89`）、実機でプローブより前に持ち上げが起きたので revert した。別アプリ対照（Picasa）で、事象は Spica 固有と確定した。次の候補は 7（未実施・要合意）。

---

## §4 タスク

進捗（2026-09-21）: C1 / A1 / A2 / A3 は実装・コミット済み。C1 Step 6 は項目 1 合格、項目 2 は手動では成立しない（窓生成が起動 ~0.2–0.5 秒後で、0.5 秒以内に別アプリをクリックできない）、項目 3 は C4 のビルドで併せて実施。実機再現（§1.5）を受けて C4 を実装済み（実機検証待ち）。C2 / C3 は条件付きのため未着手。行為者の判別実験 A/B（§1.5）を実施した。当初は「起動時プローブが必要条件」と判断したが、発生率の低い条件で行っていて判別力が無かったので訂正した。実験 B のスイッチ（`e1de747`）は、製品に文書化されない環境変数を残さないため revert した。§3 の 6 のうち C3 を実装した（`835fd89`）。実機 A/B の 1 回目は判定不能で、2 回目は C3 でもプローブより前に持ち上げが起きたので打ち切り、revert した（W4 = 非採用）。Picasa を同じ条件で起動した対照で、事象は Spica 固有と確定した。C4 は実機で合格した（`z_raise` 17 回すべて `ok:true`、1 秒以上覆われた起動は 0 件）。C1（W2）は 260 起動で一度も発動しなかったので撤去した。PR #332 は W3 と診断基盤でまとめ、§3 の 7 は別 PR で行う。
...
エクスプローラーから起動し、C3 のビルドと直前のビルド（対照）をブロックごとに交互に入れ替えて、それぞれ 30 起動以上。合格条件: C3 側で `window_created` の `z_above` が非 0 の起動が 0 件で、同時に取った対照側には起きていること（対照でも 0 件なら判定不能）。あわせて、一覧・サムネイルバーの出方に体感の悪化が無いこと。不合格なら `git revert`。
  ...
  - 2 回目（高頻度条件）: いつもの窓から、毎回その日まだ起動していない NAS フォルダへ移動して最初に起動する。10 起動ずつ C3 → 対照 → C3 → 対照で、各 20 起動の予定だった。対照の見込みは ~30%（§1.5 の 6/18）。
  - 結果: 1 ブロック目（C3、同じ UNC フォルダで 10 起動）で 1/10 の持ち上げ。プローブが走る前だったので**不合格**とし、打ち切って revert した（§1.5）。
...
- [x] **Step 6: e2e** `npm run bench:build` → `npm run test:e2e` 2 回連続 green（W2 撤去後にも再実施）
- [x] **Step 7: 実機検証（ユーザー）** — インストーラを更新し、エクスプローラーから 30 回以上起動（ローカル・NAS）。合格条件: 「Spica が前面のまま、起動元エクスプローラーが `above` に 1 秒以上残る」起動が 0 件。`perf.log` の `z_raise ... ok:true` が症状の出た起動と対応していること。あわせて C1 Step 6 の項目 3（ウインドウ化・F11）を実施。
  - 結果（2026-09-21、ローカル・NAS あわせて 200 起動超）: `z_raise` 17 回すべて `ok:true` で、症状の出た起動と 1 対 1 に対応した。ウォッチャーで覆われていた時間は 0.44〜0.49 秒、1 秒以上は 0 件。ウインドウ化・F11 の確認は、W2 撤去後のビルドでユーザーが行う。
...
- （2026-09-21 夕方）上の C4 の条件は満たした（§4 進捗）。C1 は撤去したので、上の C1 関連の条件は適用しない。PR #332（W3 + 診断基盤）のマージで本件をクローズし、持ち上げ自体を無くす調査（§3 の 7）は別 PR とする。
