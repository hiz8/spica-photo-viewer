...
10. キーボードで Tab してバーに入るとバーが表示され、フォーカスが中にある間は消えない。
11. ウインドウ化モード（W1）で、見えない下部 80px 帯が画像の上でもマウス入力を捉えることが従来どおり保たれている。
12. 画像をドラッグで下の帯まで動かすとバーが出る（ホバーとして扱われるため、これは許容される挙動）。
