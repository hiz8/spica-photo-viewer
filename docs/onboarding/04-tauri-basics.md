...
> **Tauri v1 との違い**: 検索で出てくる Tauri v1 の記事には「自作コマンドを呼ぶには `tauri.allowlist` に明示的に追加する必要がある」と書かれているものが多く、混乱の元になりがちです。**v2 では allowlist は廃止され、capability + permission モデルに置き換わっています**。自作コマンドは `core:default` 配下で自動的に許可されるため、v1 のような個別追加作業は不要です。本プロジェクトは Tauri v2.10 (`Cargo.toml:21`) を使用しています。
