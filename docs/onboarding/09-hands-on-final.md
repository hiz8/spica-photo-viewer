...
`cancelled` フラグはレースコンディション対策です。ナビゲーションが速い場合、古い invoke の結果で新しい画像のサイズを上書きしないようにしています。目的としては [06 章の二段階画像ロード戦略](./06-frontend-walkthrough.md#二段階画像ロード戦略の実装) と同じですが、06 章では `AbortController` を使ってキャンセルしており、ここでは **簡易版として** boolean フラグで古い結果を無視しています。
...
- **async 処理でレース対策を入れる** パターン（このハンズオンでは説明簡略化のため `cancelled` フラグを使ったが、このコードベースでは `AbortController` ベースのキャンセルもよく使います）
