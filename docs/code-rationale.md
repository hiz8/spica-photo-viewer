...
- **「覆っている」の判定**: 自分より z が上で、可視・非最小化・`WS_EX_TOOLWINDOW` でない・
  ...
  何も言えないので動かない。
- **誰が・何がエクスプローラーを持ち上げるかは未確定**（plan §1.5）。発生は、エクスプローラーで
  フォルダを移動した直後の最初の起動に強く偏る（その日まだ起動していない NAS フォルダで 6/18、
  同じフォルダを繰り返し起動すると 2/63）。起動時の `explorer_sort` COM プローブが表示と
  重なることを疑って実験 A/B を行ったが、発生率の低い条件で行ったので判別力が無かった。
  その後、重なりをなくす W4 を入れて検証している。この対策は行為者に依存しないので、W4 の
  成否とは独立に成り立つ。旧 `maximize_window`（`ShowWindow(SW_MAXIMIZE)`）が z も直していた、というのは
  推定であり、確定した回帰機構（W2）とは別扱い。
...
**起動時のフォルダ先読み（と Explorer ソートプローブ）はウインドウ生成の後に始める**

W3 の状態（前面は自分・z だけ起動元エクスプローラーの下）を引き起こしているのは、起動時の
`explorer_sort` COM プローブ（エクスプローラーの UI スレッドへの RPC）が最初のウインドウ表示と
重なることだと疑っている。ただし未確定（plan §1.5。実験 A/B は発生率の低い条件で行ったので
判別力が無かった）。重なりをなくして事象が消えるかは、実機 A/B で確かめている
（plan Task C3 Step 3）。消えるなら、W3 が直すまでの ~0.5 秒エクスプローラーが上に居る
見た目も出なくなる。プローブはフォルダ走査の中で起動される（`scan_folder` →
`spawn_detect`）ので、`setup` ではサムネイル先読みだけをウインドウ生成前に始め、フォルダ
先読みは `.build()` が返った後（`window_created` の後）に始める。

- **サムネイル先読みは前のまま**: 首枚の表示はフォルダ走査に依存せず、WebView2 初期化の待ちと
  重ねる意味が大きい。エクスプローラーには触れない。
- **代償**: フォルダ一覧が揃う時刻（`folder:scanned`）は遅れうる。フロントが一覧を求めるのは
  ページロードと React マウントの後なので、遅れは走査時間のうちその間に収まらない分だけ。
  走査の長い大フォルダ・NAS では、遅れはほぼウインドウ生成の分（~300〜500ms）そのままになる。
  実測（2026-09-21、ローカル 30 枚、`e2e/scripts/profile-startup.mjs` で直前のビルドの exe と
  交互に 9 起動ずつ、`window_created` 起点）: 走査は `window_created` +0〜2ms に始まる。
  ウインドウ生成が速い回（起動から ~0.5 秒）では `folder:scanned` に差が無く（前 +88〜128ms、
  後 +90〜109ms）、遅い回（0.9〜1.5 秒）では中央値 +241ms → +281ms（+40ms）。首枚の
  paint は変わらない。
- **プローブだけ遅らせる案は、まず採らない**: 走査はディレクトリ列挙とプローブを並行させ、
  最後にプローブの結果（表示順）を待つ（`join()`）。プローブだけを遅らせれば、列挙の長い
  フォルダでは遅れを隠せる。ただし走査に「ウインドウ生成まで待つ」合図を通す必要があり、
  先に「重ならなければ事象が消える」ことを最小の変更で確かめる。大フォルダでの遅れが
  問題になったら次の一手とする。
- W3 は残す。行為者に依存しない保険であり、W4 の効果の統計的な確認も起動 30〜60 回規模に
  とどまる。実機 A/B で効果が確認できなければ W4 は revert する（起動時間の代償だけが残るため）。

参照元: `src-tauri/src/lib.rs`（`setup` の `start_thumbnail` / `start_folder`）、
`src-tauri/src/commands/startup.rs`、plan §1.5 / §3 の 6 / Task C3
