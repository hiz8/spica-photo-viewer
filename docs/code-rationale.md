...
- **「覆っている」の判定**: 自分より z が上で、可視・非最小化・`WS_EX_TOOLWINDOW` でない・
  ...
  何も言えないので動かない。
- **持ち上げの条件は、起動時の `explorer_sort` COM プローブとウインドウ表示の重なり**
  （2026-09-21 の実験 A/B、plan §1.5）。プローブを止めたビルドでは 0/30 だった。同じビルド・
  同じ窓でプローブを有効にした対照は 2/30、プローブ有効の累計は 7/65。一方、Spica の窓を
  出さずにプローブだけを走らせても、エクスプローラーは持ち上がらなかった（0/30）。統計的な
  強さは中程度（Fisher の片側検定で p ≈ 0.06）。どの COM 呼び出しがなぜ z を動かすのかは
  未解明。この対策は行為者に依存しないので、プローブ側の対策（plan §3 の 6、提案）とは独立に
  成り立つ。旧 `maximize_window`（`ShowWindow(SW_MAXIMIZE)`）が z も直していた、というのは
...
W3 の状態（前面は自分・z だけ起動元エクスプローラーの下）は、起動時の `explorer_sort` COM
プローブが最初のウインドウ表示と重なったときにだけ起きる（2026-09-21 の実験 A/B、
plan §1.5）。重なりをなくせば事象そのものが起きず、W3 が直すまでの ~0.5 秒エクスプローラーが
上に居る見た目も出ない。プローブはフォルダ走査の中で起動される（`scan_folder` →
