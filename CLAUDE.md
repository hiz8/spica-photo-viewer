...
npm run lint:fix           # Auto-fix lint issues (oxlint)
npm run format:fix         # Auto-fix formatting (oxfmt)
...
- **機能を持つコメントは削除しない**: `oxlint-disable-next-line`（`useImagePreloader.ts` ほか 7 ファイルで使用中）/ `@ts-expect-error`（`vite.config.ts` で使用中）/ `/// <reference ... />`（`vite-env.d.ts` の唯一の行）。`// @vitest-environment` と `/* @__PURE__ */` は現在このリポジトリでは未使用だが、導入された場合も同様に扱う。
- コメントのみの変更は `npm run verify:comments -- <base-ref>` で検証する（TS/TSX は esbuild でコード一致を機械確認、Rust は差分の行検査）。exit 1 はコード変更の検出（ファイル削除、`git show` 失敗、base-ref を解決できない場合も含む）。exit 2 は自動判定できなかった行の目視確認を求めるもので、原因は 2 つある: (a) `//` を含む変更行（行末コメントか、文字列中の `//` かを機械的に区別できない）、(b) 範囲内で新規追加された TS/TSX ファイル（比較対象の版が無く esbuild 検査を行えない）。いずれも目視で確認できれば受理してよい。
